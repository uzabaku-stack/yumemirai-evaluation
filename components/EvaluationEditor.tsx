  ...
  function bulkSet(targetItems: EvaluationItem[], score: number, mode: "empty" | "overwrite", message: string) {
    ...
    setScoreValues((current) => {
      ...
      for (const item of targetItems) {
        const currentScore = current[item.id];
        const hasScore = currentScore !== null && currentScore !== undefined;
        const isNotApplicable = !!notApplicableValues[item.id];
    ...
    setNotApplicableValues((current) => {
      const next = { ...current };
      for (const item of targetItems) {
        const currentScore = scoreValues[item.id];
        const hasScore = currentScore !== null && currentScore !== undefined;
        const isNotApplicable = !!current[item.id];
        if (mode === "empty" && (hasScore || isNotApplicable)) continue;
        next[item.id] = false;
      }
      return next;
    });
    setBulkMessage(mode === "empty" ? changedCount + "件の未入力項目を" + score + "点にしました。入力済み項目と評価しない項目は変更していません。" : changedCount + "件の評価項目を" + score + "点で上書きしました。");
  }

  function bulkAll(score: number, mode: "empty" | "overwrite") {
    bulkSet(items, score, mode, mode === "empty" ? "表示中の未入力項目だけ" + score + "点にします。よろしいですか？" : "表示中の全評価項目を" + score + "点に上書きします。よろしいですか？");
  }

  function bulkSection(section: string, sectionItems: EvaluationItem[], score: number) {
    bulkSet(sectionItems, score, "overwrite", "「" + section + "」の全評価項目を" + score + "点に上書きします。よろしいですか？");
  }
