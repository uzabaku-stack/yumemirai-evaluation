...
export function DirectorPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.error || "パスワードを変更できませんでした。");
      }
      ...
      setError(submitError instanceof Error ? submitError.message : "パスワードを変更できませんでした。");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <form onSubmit={submit} className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="space-y-2">
          ...
          <input
            ...
            className="h-14 w-full rounded border border-slate-300 px-4 text-lg"
          />
        </label>
        <label className="space-y-2">
      ...
      <button
        disabled={saving}
        className="mt-5 flex min-h-14 items-center gap-2 rounded bg-clinic px-6 py-4 text-lg font-bold text-white disabled:opacity-60"
      >
        <Save size={20} />
        {saving ? "変更中..." : "変更"}
      </button>
