...
const storageKey = "yumemirai_bonus_calculator_v3";
const legacyStorageKey = "yumemirai_bonus_calculator_v2";
const overallAdjustmentRates = [0, 5, 10, 20];
...
function defaultRowInput(): RowInput {
  return { baseSalary: "", baseBonus: "", overallAdjustmentRate: "0", memo: "" };
}
...
function normalizeRowInput(input?: LegacyRowInput): RowInput {
  ...
  return {
    ...
    overallAdjustmentRate: input?.overallAdjustmentRate ?? input?.absenceRate ?? fallback.overallAdjustmentRate,
...
export function BonusCalculator({ staff }: Props) {
  ...
  const calculated = useMemo(() => {
    const baseRows = staff.map((person) => {
      ...
      const overallAdjustmentMinusRate = numberValue(input.overallAdjustmentRate);
      const coefficient = evaluationCoefficient(person.bonusScore);
      const overallAdjustmentMultiplier = Math.max(0, 1 - overallAdjustmentMinusRate / 100);
      const weight = coefficient * overallAdjustmentMultiplier;
      const baseModeBonus = baseBonus * weight;
      return { person, input, baseBonus, coefficient, overallAdjustmentMinusRate, overallAdjustmentMultiplier, weight, baseModeBonus, finalBonus: baseModeBonus };
    });
    if (mode === "pool") {
  ...
  function exportCsv() {
    const header = ["計算モード", "スタッフ名", "職種", "基本給", "スタッフ評価平均", "評価標準化", "賞与反映評価", "評価係数", "総合補正マイナス率", "総合補正率", "基準賞与額", "最終賞与額", "メモ"];
    const body = calculated.map((row) => [
      ...
      row.coefficient,
      row.overallAdjustmentMinusRate + "%",
      row.overallAdjustmentMultiplier,
      row.baseBonus,
      ...
      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">保存処理や評価データは変更しません。入力内容はこの端末に保存できます。</p>
            <p className="mt-1 text-sm text-slate-600">総合補正は休暇日数だけでなく、今後の各種補正も含めて調整できる項目です。</p>
          </div>
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[1380px] text-left">
            <thead><tr className="border-b text-sm text-slate-500"><th className="py-3">スタッフ名</th><th>職種</th><th>基本給</th><th>スタッフ評価平均</th><th>評価標準化</th><th>賞与反映評価</th><th>評価係数</th><th>総合補正</th><th>基準賞与額</th><th>最終賞与額</th><th>メモ</th></tr></thead>
            <tbody>
              {calculated.map((row) => (
                <tr key={row.person.id} className="border-b last:border-0 align-top">
                  <td className="py-4 pr-3 font-bold text-ink">{row.person.name}</td>
                  <td className="py-4 pr-3"><span className="rounded bg-slate-100 px-3 py-2 text-sm font-bold text-slate-700">{row.person.role}</span></td>
                  <td className="py-3 pr-3"><input inputMode="numeric" value={row.input.baseSalary} onChange={(event) => update(row.person.id, { baseSalary: event.target.value })} className="h-12 w-36 rounded border border-slate-300 px-3 text-right" placeholder="例 250000" /></td>
                  <td className="py-4 text-lg font-bold text-ink">{row.person.averageScore === null ? "-" : row.person.averageScore.toFixed(2)}</td>
                  <td className="py-4 text-lg font-bold text-clinic">{row.person.standardizedScore === null ? "-" : row.person.standardizedScore.toFixed(2)}</td>
                  <td className="py-4 text-lg font-bold text-clinic">{row.person.bonusScore === null ? "-" : row.person.bonusScore.toFixed(2)}</td>
                  <td className="py-4 font-bold">{row.coefficient.toFixed(1)}</td>
                  <td className="py-3 pr-3">
                    <label className="sr-only" htmlFor={"overall-adjustment-" + row.person.id}>総合補正</label>
                    <select id={"overall-adjustment-" + row.person.id} value={row.input.overallAdjustmentRate} onChange={(event) => update(row.person.id, { overallAdjustmentRate: event.target.value })} title="総合補正。休暇日数や将来の各種補正を含めたマイナス率です。" className="h-12 w-36 rounded border border-slate-300 px-3">
                      {overallAdjustmentRates.map((rate) => <option key={rate} value={String(rate)}>{rate}% → {(1 - rate / 100).toFixed(2)}</option>)}
                    </select>
                  </td>
                  <td className="py-3 pr-3"><input inputMode="numeric" value={row.input.baseBonus} onChange={(event) => update(row.person.id, { baseBonus: event.target.value })} disabled={mode !== "base"} className="h-12 w-36 rounded border border-slate-300 px-3 text-right disabled:bg-slate-100 disabled:text-slate-400" placeholder="例 300000" /></td>
                  <td className="py-4 pr-3 text-xl font-bold text-clinic">{yen(row.finalBonus)}</td>
                  <td className="py-3"><textarea value={row.input.memo} onChange={(event) => update(row.person.id, { memo: event.target.value })} className="min-h-12 w-64 rounded border border-slate-300 p-3" placeholder="メモ" /></td>
                </tr>
              ))}
            </tbody>
            <tfoot><tr className="bg-mint/60"><td colSpan={9} className="px-3 py-4 text-right text-lg font-bold">合計賞与額</td><td className="px-3 py-4 text-xl font-bold text-clinic">{yen(totalBonus)}</td><td /></tr></tfoot>
          </table>
        </div>
      </section>

      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        <h2 className="text-xl font-bold">評価標準化の考え方</h2>
        <p className="mt-2 text-sm leading-7 text-slate-600">スタッフ評価平均とは別に、評価者ごとの甘め・厳しめの傾向を控えめに補正した「評価標準化」を表示します。賞与計算では「賞与反映評価」を使用します。補正ロジックは <code className="rounded bg-slate-100 px-2 py-1">lib/evaluationStandardization.ts</code> に独立しています。</p>
      </section>

      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        <h2 className="text-xl font-bold">評価係数</h2>
        <div className="mt-4 grid gap-3 md:grid-cols-5">{["4.5以上: 1.2", "4.0以上: 1.1", "3.5以上: 1.0", "3.0以上: 0.9", "3.0未満: 0.8"].map((item) => <div key={item} className="rounded bg-slate-50 p-4 text-center font-bold text-ink">{item}</div>)}</div>
      </section>
    </div>
  );
}
