...
function period(row: { cycle: { startDate: string; endDate: string } }) {
  return row.cycle.startDate + " ～ " + row.cycle.endDate;
}
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-slate-600">評価回ごとの記録を確認し、過去の帳票を再出力できます。</p>
        </div>
        ...
        <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {staffList.map((person) => (
              <Link key={person.id} href={"/evaluation-history?staffId=" + person.id} className={(person.id === staffId ? "bg-clinic text-white" : "border border-clinic text-clinic") + " rounded px-4 py-3 font-bold"}>
                {person.name}
              </Link>
            ))}
          </div>
      ...
      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        <div className="flex items-center gap-2">
          <History className="text-clinic" />
          <h2 className="text-xl font-bold">{isDirector ? (staff?.name ?? "スタッフ") : "自分"}の評価履歴</h2>
        </div>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[860px] text-left">
            <thead>
              <tr className="border-b text-sm text-slate-500">
                <th className="py-3">評価回名</th>
                <th>評価期間</th>
                <th>総合平均</th>
                <th>テーマ別平均</th>
                <th>コメント</th>
            ...
            <tbody>
              {histories.length ? histories.map((row) => (
                <tr key={row.cycle.id} className="border-b last:border-0 align-top">
                  <td className="py-4 font-bold">{row.cycle.name}</td>
                  <td>{period(row)}</td>
                  <td className="text-lg font-bold text-clinic">{fmt(row.overall_average)}</td>
                  <td>
                    <div className="flex flex-wrap gap-2">
                      {row.themes.map((theme) => <span key={theme.theme} className="rounded bg-slate-100 px-3 py-1 text-sm">{theme.theme}: {fmt(theme.average)}</span>)}
                    </div>
                  </td>
