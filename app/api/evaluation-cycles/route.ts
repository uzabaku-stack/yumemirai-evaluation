...
export async function POST(request: Request) {
  ...
  const cycle = await createEvaluationCycle({ name: String(body.name ?? ""), startDate: String(body.startDate ?? ""), endDate: String(body.endDate ?? ""), status: body.status as EvaluationCycleStatus });
