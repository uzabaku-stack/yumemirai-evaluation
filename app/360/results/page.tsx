...
function hrefFor(params: Search, patch: Partial<Search>) {
  ...
  for (const [key, value] of Object.entries(merged)) if (value !== undefined && value !== "") next.set(key, String(value));
...
function buildStaffReports(rows: StaffSummary[], periodName: string): StaffExportReport[] {
  ...
  return rows.map((row) => {
    const directorEvaluation = [...row.evaluations].filter((evaluation) => evaluation.evaluation_type === "director").sort((a, b) => (b.updated_at || b.created_at || "").localeCompare(a.updated_at || a.created_at || ""))[0];
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-slate-600">スタッフごとの評価結果・履歴・削除・帳票出力を管理します。</p>
        </div>
      ...
      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {cycles.map((cycle) => <Link key={cycle.id} href={hrefFor(params, { cycleId: String(cycle.id), staffId: undefined })} className={(selectedCycle?.id === cycle.id ? "bg-clinic text-white" : "border border-clinic text-clinic") + " rounded px-4 py-3 font-bold"}>{cycle.name}</Link>)}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {completedStaffRows.map((row) => <Link key={row.staff.id} href={hrefFor(params, { staffId: String(row.staff.id) })} className={(selectedStaffId === row.staff.id ? "bg-clinic text-white" : "border border-clinic text-clinic") + " rounded px-4 py-3 font-bold"}>{row.staff.name}</Link>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="rounded border border-teal-900/10 bg-white p-5 shadow-soft"><div className="flex items-center gap-2 text-sm font-bold text-slate-500"><FileText size={18} />評価件数</div><div className="mt-2 text-3xl font-bold text-clinic">{listedEvaluations.length}</div></div>
        <div className="rounded border border-teal-900/10 bg-white p-5 shadow-soft"><div className="flex items-center gap-2 text-sm font-bold text-slate-500"><UserCheck size={18} />評価完了</div><div className="mt-2 text-3xl font-bold text-clinic">{completion.completedCount} / {completion.targetStaffCount}</div></div>
        <div className="rounded border border-teal-900/10 bg-white p-5 shadow-soft"><div className="flex items-center gap-2 text-sm font-bold text-slate-500"><History size={18} />評価期間</div><div className="mt-2 text-xl font-bold text-ink">{selectedCycle?.name ?? "-"}</div></div>
        <div className="rounded border border-teal-900/10 bg-white p-5 shadow-soft"><div className="flex items-center gap-2 text-sm font-bold text-slate-500"><MessageSquareText size={18} />コメントあり</div><div className="mt-2 text-3xl font-bold text-clinic">{staffRows.filter((row) => row.evaluations.some((evaluation) => visibleComments(evaluation.comments).length)).length}</div></div>
      </section>
      ...
      <section className="rounded border border-teal-900/10 bg-white p-5 shadow-soft">
        ...
        <p className="mt-1 text-sm text-slate-600">PDF出力（1人用）、Excel一覧（全スタッフ）、Excel評価シート（1人用）、CSV出力をここから操作できます。</p>
