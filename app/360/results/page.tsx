...
export default async function EvaluationResultsPage({ searchParams }: { searchParams?: Promise<Search> }) {
  ...
  const listedEvaluations = allEvaluations
    .filter((evaluation) => selectedCycle ? evaluation.evaluation_cycle_id === selectedCycle.id || (!evaluation.evaluation_cycle_id && evaluation.evaluation_month === selectedCycle.startDate.slice(0, 7)) : true)
    .sort((a, b) => (b.updated_at || b.created_at || "").localeCompare(a.updated_at || a.created_at || ""));
  const selectedStaffId = query.staffId ? Number(query.staffId) : null;
  const staffRows = selectedStaffId ? summary.staff_summaries.filter((row) => row.staff.id === selectedStaffId) : summary.staff_summaries;
