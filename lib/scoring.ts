...
export function rankForAverage(average: number) {
  if (average >= 4.3) return "A";
  if (average >= 3.6) return "B";
...
export function calculateSummary(items: EvaluationItem[], scores: EvaluationScore[]) {
  ...
  for (const item of items) {
    ...
    const value = byItem.get(item.id) ?? 1;
    const current = sections.get(item.section_name) ?? { total: 0, count: 0 };
    current.total += value;
    current.count += 1;
    sections.set(item.section_name, current);
  }

  const sectionScores = Array.from(sections.entries()).map(([section, value]) => ({
    section,
    total: value.total,
    max: value.count * 5,
...
export function parseComments(raw: string | null | undefined) {
